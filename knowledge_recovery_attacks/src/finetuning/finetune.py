...
if __name__ == "__main__":
    ...
    model_str = ".".join(args.model.split("/")[2:-2])
    run_name = f"{model_str}__{args.dataset}__{args.n_samples}"
    ...
    # Get dataset
    dataset = DATASET_REGISTRY[args.dataset](limit=args.n_samples if args.n_samples != -1 else None, system_prompt=args.system_prompt)
    ...
    trainer = SFTTrainer(
        ...
        args = SFTConfig( #these can be retrieved later on so no need to make them into Global constants for saving purposes
    ...
    bench = Benchmark(
        output_dest,
        tasks=["mmlu", "wmdp-bio", "wmdp-cyber"],
        wandb_project=os.getenv("WANDB_PROJECT", "temp"),
        run_name=f"finetuning/dataset.{args.dataset}/unlearned_model.{model_str}/n_samples.{args.n_samples}",
        save_requests=False,
