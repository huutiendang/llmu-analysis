    ...
    for i in tqdm(range(len(mmlu_dataset)), smoothing= 0.0):
        ...
        for layer in args.layers:
            mmlu_activations[layer] = torch.vstack((mmlu_activations[layer], 
                                                 model.layer_out[int(layer)][0].cpu().float()))
...
def test_orthogonalization(args, model, tokenizer, noise_directions: dict, output_dir: str) -> None:
    model_alias = ".".join(args.unlearned_model.split("/")[2:-2])
    attack_layers = "all" if len(args.layers) == model.config.num_hidden_layers else args.layers
    ...
    for noise_type in noise_directions:
        bench = Benchmark(
            ...
            config=dict(direction_type=noise_type,
                        **vars(copy(args))),
            run_name=f"orthogonalization/{args.task}/unlearned_model.{model_alias}/noise_type.{noise_type}/layers.{attack_layers}",
            save_requests=False,
...
if __name__ == "__main__":
    ...
    # create output dir
    model_alias = ".".join(args.unlearned_model.split("/")[2:-2])
    output_dest = os.path.join(OUTPUT_DIR, "orthogonalization", "data", model_alias + "_" + args.task)
