...
class AttackSet:
    ...
    def serialize(self):
        ...
        s.loss = s.loss.cpu().float().numpy()
        s.attack_loss = s.attack_loss.cpu().float().numpy()
        s.xentropy = s.xentropy.cpu().float().numpy()
        s.repetition = s.repetition.cpu().float().numpy()
...
class Victim:
    ...
    def __post_init__(self):
        ...
        if self.cfg is None:
            try:
                ...
                default_model_names = util.model_configs.keys()
                default_model_names = [name.replace("default/", "") for name in default_model_names]
                model_name = [name for name in default_model_names if name in self.name][0]

                self.cfg = util.model_configs[f"default/{model_name}"]
                self.cfg.model_name = self.name
    ...
    def tokenizer(self):
        if self._tokenizer is None:
            if self.cfg.tokenizer_name is None:
                self.cfg.tokenizer_name = self.cfg.model_name
            self._tokenizer = util.load_tokenizer(self.cfg.tokenizer_name)
