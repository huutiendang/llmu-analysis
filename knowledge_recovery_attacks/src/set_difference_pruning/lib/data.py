...
def get_wikitext2(nsamples, seed, seqlen, tokenizer, disentangle=False, dataset="wikitext"):
    traindata = load_dataset("json", data_files=f"./data/wikitext.jsonl", split="train")
    random.seed(seed)
    # Encode datasets
    trainloader = []
    for i in range(nsamples):
        inp = tokenizer(traindata["text"][i], return_tensors="pt")
        inp = inp.input_ids[:, :seqlen]
        tar = inp.clone()
        tar[:, :1] = -100  # TODO: first 1 are surrogate prompt tokens
        trainloader.append((inp, tar))
...
def get_wmdp(nsamples, seed, seqlen, tokenizer, disentangle=False, dataset="wmdp-bio-forget"):
    traindata = load_dataset("json", data_files=f"./data/{dataset}.jsonl", split="train")
    random.seed(seed)
    # Encode datasets
    trainloader = []
    for i in range(nsamples):
...
def get_loaders(
    ...
    if name.startswith("wmdp"):
        return get_wmdp(nsamples, seed, seqlen, tokenizer, disentangle, dataset=name)
