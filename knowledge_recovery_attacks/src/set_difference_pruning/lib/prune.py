...
def prune_wandg_set_difference(
    ...
    if args.use_diff or args.recover_from_base:
        assert model_base is not None
        layers_base = model_base.model.layers
    metric1 = "alpaca_cleaned_no_safety" if not prune_data.startswith("wmdp") else "wikitext"
    metric2 = prune_data
    ...
    for i in range(len(layers)):
        ...
        if not args.prune_part:
            for name in subset:
                print(f"pruning layer {i} name {name}")
                if args.model == "llama2-7b-chat-hf" or args.model == "llama3.2-3b-instruct" or args.model.startswith("zephyr-7b-beta"):
                    W_metric1 = pickle.load(
                        open(
                            f"../../results/pruning/data/{args.model}/unstructured/wandg/{metric1}/wanda_score/W_metric_layer_{i}_name_model.layers.{i}.{name}_weight.pkl",
                            "rb",
                        )
                    )
                    W_metric2 = pickle.load(
                        open(
                            f"../../results/pruning/data/{args.model}/unstructured/wandg/{metric2}/wanda_score/W_metric_layer_{i}_name_model.layers.{i}.{name}_weight.pkl",
                            "rb",
                        )
                    )
