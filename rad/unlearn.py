...
def run_rmu(
    ...
    if args.direction in ["truth", "refusal", "instruction"]:
        probe = load_probes(path=f"data/target_directions/lp/{args.direction}/{args.direction}_direction_probe_layer-7.pkl")
        # theta = torch.tensor(probe, dtype=torch.bfloat16)
    ...
    elif args.direction in ["mistral_truth"]:
    ...
    elif args.direction in ["llama3_refusal"]:
        probe = load_probes(path=f"data/target_directions/lp/refusal/{args.direction}_direction_probe_layer-7.pkl")
        # theta = torch.tensor(probe, dtype=torch.bfloat16)
        theta = torch.tensor(probe.coef_[0], dtype=torch.bfloat16)
        concept_vector = theta / torch.norm(theta)
        concept_vector = concept_vector.to(updated_model.device)
    # elif args.direction in ["4_shot", "8_shot", "16_shot"]:
    #     probe = load_probes(path=f"data/target_directions/lp/fewshot/{args.direction}_direction_probe_layer-7.pkl")
    #     # theta = torch.tensor(probe, dtype=torch.bfloat16)
    #     theta = torch.tensor(probe.coef_[0], dtype=torch.bfloat16)
    #     concept_vector = theta / torch.norm(theta)
    #     concept_vector = concept_vector.to(updated_model.device)
    
    elif args.direction in ["country2capital", "person2language"]:
        probe = load_probes(path=f"data/target_directions/lp/instruction/knowledge/{args.direction}/{args.direction}_direction_probe_layer-7.pkl")
        # theta = torch.tensor(probe, dtype=torch.bfloat16)
        theta = torch.tensor(probe.coef_[0], dtype=torch.bfloat16)
        concept_vector = theta / torch.norm(theta)
        concept_vector = concept_vector.to(updated_model.device)
    
    elif args.direction in ["antonyms", "present2past"]:
        probe = load_probes(path=f"data/target_directions/lp/instruction/linguistic/{args.direction}/{args.direction}_direction_probe_layer-7.pkl")
        # theta = torch.tensor(probe, dtype=torch.bfloat16)
        theta = torch.tensor(probe.coef_[0], dtype=torch.bfloat16)
        concept_vector = theta / torch.norm(theta)
        concept_vector = concept_vector.to(updated_model.device)
        
    elif args.direction in ["mistral_antonyms"]:
        probe = load_probes(path=f"data/target_directions/lp/instruction/linguistic/antonyms/{args.direction}_direction_probe_layer-7.pkl")
        # theta = torch.tensor(probe, dtype=torch.bfloat16)
        theta = torch.tensor(probe.coef_[0], dtype=torch.bfloat16)
        concept_vector = theta / torch.norm(theta)
        concept_vector = concept_vector.to(updated_model.device)
    
    elif args.direction in ["mistral_present2past"]:
        probe = load_probes(path=f"data/target_directions/lp/instruction/linguistic/present2past/{args.direction}_direction_probe_layer-7.pkl")
        # theta = torch.tensor(probe, dtype=torch.bfloat16)
        theta = torch.tensor(probe.coef_[0], dtype=torch.bfloat16)
        concept_vector = theta / torch.norm(theta)
        concept_vector = concept_vector.to(updated_model.device)
        
    elif args.direction in ["mistral_country2capital"]:
        probe = load_probes(path=f"data/target_directions/lp/instruction/knowledge/country2capital/{args.direction}_direction_probe_layer-7.pkl")
        # theta = torch.tensor(probe, dtype=torch.bfloat16)
        theta = torch.tensor(probe.coef_[0], dtype=torch.bfloat16)
        concept_vector = theta / torch.norm(theta)
        concept_vector = concept_vector.to(updated_model.device)
    
    elif args.direction in ["mistral_person2language"]:
        probe = load_probes(path=f"data/target_directions/lp/instruction/knowledge/person2language/{args.direction}_direction_probe_layer-7.pkl")
        # theta = torch.tensor(probe, dtype=torch.bfloat16)
        theta = torch.tensor(probe.coef_[0], dtype=torch.bfloat16)
        concept_vector = theta / torch.norm(theta)
        concept_vector = concept_vector.to(updated_model.device)
    
    elif args.direction in ["sentiment"]:
        probe = load_probes(path=f"data/target_directions/lp/instruction/{args.direction}/{args.direction}_direction_probe_layer-7.pkl")
        # theta = torch.tensor(probe, dtype=torch.bfloat16)
        theta = torch.tensor(probe.coef_[0], dtype=torch.bfloat16)
        concept_vector = theta / torch.norm(theta)
        concept_vector = concept_vector.to(updated_model.device)
    
    elif args.direction in ["mistral_positive"]:
    ...
    elif args.direction in ["mistral_negative"]:
        probe = load_probes(path=f"data/target_directions/lp/sentiment/mistral_positive_direction_probe_layer-7.pkl")
        # theta = torch.tensor(probe, dtype=torch.bfloat16)
        theta = torch.tensor(probe.coef_[0], dtype=torch.bfloat16)
        concept_vector = -theta / torch.norm(theta)
    ...
    with tqdm.tqdm(total=num_batches) as pbar:
        ...
        for idx in range(num_batches):
            ...
            h_t = frozen_forget_activations + args.steering_coeff_list[topic_idx] * concept_vector
...
def get_args():
    ...
    parser.add_argument("--direction", type=str, default="random", choices=["mistral_person2language","mistral_country2capital", "mistral_present2past","mistral_antonyms", "llama3_refusal", "mistral_refusal","mistral_positive", "mistral_negative", "mistral_truth", "sentiment", "snippets", "sst2", "present2past", "antonyms", "person2language", "country2capital","4_shot", "8_shot", "16_shot", "instruction", "law", "positive", "negative", "random", "truth", "refusal"])
...
if __name__ == "__main__":
    ...
    path = f"checkpoints/rm/rad/factual/knowledge/person2language_direction/{args.model_name_or_path}_alpha-{int(args.alpha[0])}-{int(args.alpha[1])}_coeffs-{float(args.steering_coeff_list[0])}-{float(args.steering_coeff_list[1])}_batches-{args.max_num_batches}_layer-{args.layer_id}_component-{args.sub_component}_nu-{args.nu}"
